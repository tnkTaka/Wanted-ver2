...
import (
	...
	"github.com/tnkTaka/go-LevenshteinDistance/distance"
...
func CreateQuestion(questionTitle, questionBody, jwtToken string, studentId, questionGenre int) (int, error) {
	...
	if err != nil {
		...
		if err := db.Exec("INSERT INTO `questions`(`student_id`, `genre_id`, `question_title`, `question_body`) VALUES (?,?,?,?)", question.StudentId, question.GenreId, question.QuestionTitle, question.QuestionBody).Error; err != nil {
			return 0, err
		} else {
			db.Raw("SELECT question_id FROM questions ORDER BY question_id DESC").First(&question)

			var tags []string
			m := analysis.MorphologicalAnalysis(questionBody)
			for v , _ := range m {
				tags = append(tags, v)
			}
			...
			for _, t := range tags {
				...
				db.Exec("INSERT INTO `tags`(`question_id`, `tag_name`) VALUES (?,?)", tag.QuestionId, tag.TagName)
			}
			...
			for _, question := range questions {
				...
				approximation := distance.LevenshteinDistance(questionBody, question.QuestionBody)
			...
			if ss != nil {
				...
				db.Exec("INSERT INTO `notifications`(`question_id`, `student_id`, `approximation`) VALUES (?,?,?)",notification.QuestionId, notification.StudentId, notification.approximation)
			}
...
func CreateAnswer(questionBody, jwtToken string, studentId, questionId int) (int, bool, error) {
	...
	if err != nil {
		...
		switch count {
		case 0:
			if err := db.Exec("INSERT INTO `answers`(`question_id`, `student_id`, `answer_body`) VALUES (?,?,?)",answer.QuestionId, answer.StudentId, answer.AnswerBody).Error; err != nil {
				return 0, state, err
			} else {
				state = true
